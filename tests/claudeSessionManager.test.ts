...
test("loadQueryFn is called at most once even when main and workers are started", async () => {
  ...
  // The same queryFnPromise is reused for main and all workers.
  expect(counts.loads).toBe(1);
  // But a separate query() invocation is made for each session.
  expect(counts.queries).toBe(3);
});

test("worker records the correct role name (worker:<taskId>) in the database", async () => {
  const { manager, db, sid } = setup();
...
test("closeAll is idempotent: a second call after the first is a no-op", async () => {
  const { manager } = setup();
  await manager.main();
  await manager.closeAll();
...
test("calling main() after closeAll creates a fresh session", async () => {
  ...
  // A new session is created because mainPromise was cleared by closeAll.
  expect(counts.queries).toBe(2);
