...
test("resolveAttachTarget returns undefined when no session.db is present in the directory", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-no-db-"));
  const sid = "nosession1";
  fs.mkdirSync(path.join(dir, sid), { recursive: true });
  // No session.db file — just an empty directory.
  expect(resolveAttachTarget(dir, sid, sid)).toBeUndefined();
  fs.rmSync(dir, { recursive: true, force: true });
});

test("resolveAttachTarget returns undefined when the requested forgeSessionId has no claude_sessions row", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-empty-claude-"));
  const sid = "emptysession";
  fs.mkdirSync(path.join(dir, sid), { recursive: true });
  const db = new ForgeDb(path.join(dir, sid, "session.db"));
  db.createSession("idea", sid);
  // No claude session created → findClaudeSession returns undefined.
  db.close();
  expect(resolveAttachTarget(dir, undefined, sid)).toBeUndefined();
  fs.rmSync(dir, { recursive: true, force: true });
});

test("resolveAttachTarget returns undefined when no claude_session_id has been assigned yet", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-no-csid-"));
  const sid = "pending-session";
  fs.mkdirSync(path.join(dir, sid), { recursive: true });
  const db = new ForgeDb(path.join(dir, sid, "session.db"));
  db.createSession("idea", sid);
  // Create a claude session row but leave claude_session_id as NULL (starting state).
...
test("resolveAttachTarget marks active=false when phase is FAILED", () => {
...
test("resolveAttachTarget includes forgeSessionId and role in the returned target", () => {
...
test("findTranscript returns undefined when projectsRoot does not exist", () => {
...
test("findTranscript returns undefined when projectsRoot is empty", () => {
...
test("findTranscript finds a transcript nested under any project directory", () => {
  ...
  // Create two project dirs; the session file is in the second one.
