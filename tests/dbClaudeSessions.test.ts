...
test("createClaudeSession with model field stores model correctly", () => {
  const { db, sid } = makeDb();
  const id = db.createClaudeSession(sid, "main", "/tmp/ws", { model: "claude-opus-4-5", permissionMode: "default" });
...
test("createClaudeSession without optional fields leaves model and permission_mode null", () => {
  const { db, sid } = makeDb();
...
test("listClaudeSessions without parameter returns sessions across multiple forge sessions", () => {
  const db = new ForgeDb(":memory:");
  ...
  const forgeIds = all.map((r) => r["forge_session_id"]);
  expect(forgeIds).toContain(sid1);
  expect(forgeIds).toContain(sid2);
  db.close();
});

test("updateClaudeSession updates closed_at field and sets updated_at", () => {
  const { db, sid } = makeDb();
  ...
  // updated_at should have been touched
  expect(String(row?.["updated_at"]) >= before).toBe(true);
});

test("updateClaudeSession can record an error message", () => {
  const { db, sid } = makeDb();
  const id = db.createClaudeSession(sid, "main", "/tmp/ws");
...
test("logLlmCall with taskId associates the call to the task", () => {
  const { db, sid } = makeDb();
  const taskId = db.createTask(sid, "My Task", "coding");
  db.logLlmCall(sid, {
    model: "claude-sonnet-4-6", tokensIn: 10, tokensOut: 5, costUsd: 0.01, response: "done",
...
test("logLlmCall provider is derived from model string when not explicitly given", () => {
  const { db, sid } = makeDb();
  db.logLlmCall(sid, { model: "anthropic/claude-haiku", tokensIn: 1, tokensOut: 1, costUsd: 0, response: "x" });
  expect(db.getLlmCalls(sid)[0]["provider"]).toBe("anthropic");
});

test("getToolCalls returns tool_args as a JSON string", () => {
  const { db, sid } = makeDb();
